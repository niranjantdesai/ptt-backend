...
public class BackendTestsWeb2 {
    ...
    private void POSTTest(Map<String, Object> inputBody, int expectedStatus, Map<String, Object> expectedResponse, boolean strict) throws Exception{
        ...
        if (status == expectedStatus) {
            ...
            if (expectedStatus != 201){
                ...
                expectedResponse.put("id", getIdFromStringResponse(strResponse));
    ...
    public void getUserTest() throws Exception {
        ...
        try {
            ...
            expectedResponse = new HashMap<String, Object>();
            GETTest(id + "1", 404, expectedResponse, false);
    ...
    public void createProjectTest() throws Exception {
	    ...
	    JSONAssert.assertEquals(expectedJson,strResponse, false);
            ...
            // 400 bad request
            response = createProject("testProjectName", userId + "abc");
            status = response.getStatusLine().getStatusCode();
            Assert.assertEquals(404, status);
            EntityUtils.consume(response.getEntity());
            response.close();
            // 404 user not found
            response = createProject("testProjectName", userId + "666");
            status = response.getStatusLine().getStatusCode();
            Assert.assertEquals(404, status);
            EntityUtils.consume(response.getEntity());
            response.close();
