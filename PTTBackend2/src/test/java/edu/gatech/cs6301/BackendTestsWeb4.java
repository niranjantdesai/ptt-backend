...
public class BackendTestsWeb4 {
    ...
    // Delete a user
    private CloseableHttpResponse deleteUser(long id) throws IOException {
        // get all projects by user is and delete them
        CloseableHttpResponse response = getAllProjects(id);
        ...
        if (status == 200) {
            ...
            try {
                // Get Ids from the response
                JSONArray jsonProjects = new JSONArray(strResponse);
               // System.out.println("Projects found for user : " + Long.toString(id) + " " + Integer.toString(jsonProjects.length()));
                // For each item in array, parse the id and issue delete
                for (int i = 0; i < jsonProjects.length(); i++) {
                    long deleteID = getIdFromJSONObject(jsonProjects.getJSONObject(i));
                   // System.out.println("Deleting project id " + Long.toString(deleteID));
                    response = deleteProject(id, deleteID);
                    EntityUtils.consume(response.getEntity());
                    response.close();
                }
            } catch (JSONException je) {
