...
public class BackendTestsWeb4 {
    ...
    public void deleteUserFromFullDBTest() throws Exception {
        ...
        try {
            ...
            response = deleteUser(Long.toString(userIds[1]));
            ...
            // Try to get other users and projects, make sure they are found (code 200)
    ...
    // }
     ...
     public void addSessionUserOrProjectNotFound() throws Exception {
         ...
         try {
             ...
             response = createSession(999, 100, "2013-03-01T23:59Z", "2013-03-02T23:59Z", 1);
             status = response.getStatusLine().getStatusCode();
             Assert.assertEquals(404, status);
             ...
             response = createSession(userId1, 100, "2013-03-01T23:59Z", "2013-03-02T23:59Z", 1);
             status = response.getStatusLine().getStatusCode();
             Assert.assertEquals(404, status);
             EntityUtils.consume(response.getEntity());
             response.close();
             ...
             response = createSession(100, projectId1, "2013-03-01T23:59Z", "2013-03-02T23:59Z", 1);
             status = response.getStatusLine().getStatusCode();
             Assert.assertEquals(404, status);
             EntityUtils.consume(response.getEntity());
             response.close();
     ...
     public void addSessionInvalidParameterType() throws Exception {
         ...
         try {
             ...
             httpRequest.addHeader("accept", "application/json");
             input = new StringEntity("{\"startTime\":\"" + "2013-03-01T23:59Z" + "\"," +
                     "\"endTime\":\"" + "2013-03-02T23:59Z" +
                     "\"counter\":" + 1 + "}");
             input.setContentType("application/json");
             httpRequest.setEntity(input);
    ...
    public void updateSessionSuccessfully() throws Exception {
        ...
        try {
            ...
            response = updateSession(userId1, projectId1, sessionId1, "2013-03-01T23:59Z", "2013-03-02T23:59Z", 8);
            status = response.getStatusLine().getStatusCode();
            if (status == 200) {
                entity = response.getEntity();
            } else {
            ...
            // Check that the session is correct in the response
            String strResponse = EntityUtils.toString(entity);
            sessionId1 = Long.parseLong(getIdFromStringResponse(strResponse));
            String expectedJson = "{\"id\":" + sessionId1 + ",\"startTime\":\"2013-03-01T23:59Z\",\"endTime\":\"2013-03-02T23:59Z\", \"counter\":8}";
            // Check that the record is correct in the response
            JSONAssert.assertEquals(expectedJson,strResponse, false);
            EntityUtils.consume(response.getEntity());
            response.close();

            response = updateSession(userId1, projectId1, sessionId1, "2013-03-01T23:59Z", "2013-04-02T23:59Z", 8);
            status = response.getStatusLine().getStatusCode();
            if (status == 200) {
                entity = response.getEntity();
            } else {
            ...
            strResponse = EntityUtils.toString(entity);
            sessionId1 = Long.parseLong(getIdFromStringResponse(strResponse));
            expectedJson = "{\"id\":" + sessionId1 + ",\"startTime\":\"2013-03-01T23:59Z\",\"endTime\":\"2013-04-02T23:59Z\", \"counter\":8}";
            // Check that the record is correct in the response
            JSONAssert.assertEquals(expectedJson,strResponse, false);
            EntityUtils.consume(response.getEntity());
            response.close();
